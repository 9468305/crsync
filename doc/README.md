...
3. bsdiff/bspatch算法的性能和**内存**开销太高  
...
bsdiff 执行慢,可以接受,200M文件制作patch包需要运行在64位OS，可以接受.  
bspatch执行慢,可以接受。需要占用**n+m**内存,这点无法接受.  
当时手游产品的资源包，单个最大200M，在游戏运行同时进行200M+内存开销的patch操作，非常容易OOM.  
...
zsync 在此基础上,使用一个bitbuffer来构建"缺失表",每个分块用3个bit表示,首先做位运算,若存在再访问HashTable,否则直接跳过.  
...
测试文件: 某手游资源包, base14009.obb ~ base14012.obb 92MB～93MB  
